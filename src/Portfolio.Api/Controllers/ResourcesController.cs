...
public class ResourcesController : ControllerBase
{
    ...
    public async Task<IActionResult> ExecuteAll(){var dir=FindPath();if(!Directory.Exists(dir))return NotFound();var files=Directory.GetFiles(dir,"*.sql").OrderBy(f=>f).Select(Path.GetFileName).ToList();var results=new List<object>();using var c=_db.Create();foreach(var f in files){try{var sql=await System.IO.File.ReadAllTextAsync(Path.Combine(dir,f));await c.ExecuteAsync(sql);results.Add(new{file=f,status="ok"});}catch(Exception ex){results.Add(new{file=f,status="err",error=ex.Message});}}return Ok(new{results,totalRan=files.Count});}
